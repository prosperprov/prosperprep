...
2. On an authenticated machine with this repository checked out, run `npm ci`, then `npx wrangler d1 migrations apply prosperprep-school --remote`. This applies the schema and baseline catalog once. **Do not run `npm run db:setup` on D1:** it is a destructive local demo seed.
